...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, CreationIconComponent, MatButtonModule, MatIconModule, MatTooltipModule],
  template: `
    ...
    <div class="tool-list" role="toolbar" aria-label="Image editor tools">
      @for (tool of tools; track tool.action) {
        ...
        >
          <mat-icon fontSet="material-symbols-outlined" [attr.data-icon]="tool.icon">{{ tool.icon }}</mat-icon>
    ...
    <button mat-icon-button class="upload-button" aria-label="Upload image" matTooltip="Upload image" (click)="fileInput.click()">
      <app-creation-icon name="upload"></app-creation-icon>
      <span>Upload</span>
    </button>
  ...
  styles: [`
    ...
    .tool-button mat-icon { --creation-icon-size: 21px; width: 21px; height: 21px; font-size: 21px; }
    ...
    .upload-button app-creation-icon { width: 20px; height: 20px; }
